...
public class HeadlineFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if(Headline_REQUESTHTTP_OK==msg.what){
					...
					FragmentUtil.setDataToListView(HeadlineFragment.this.getActivity());//给listview设置数据
					FragmentUtil.setImageToViewPager(HeadlineFragment.this.getActivity());//给画廊设置数据
		...
		// 2.调用方法从网络上获取数据
		FragmentUtil.getNewsFromHttp(HeadlineFragment.this.getActivity(),handler, httpUrl, httpArg,Headline_REQUESTHTTP_OK);
		return view;
	}
