...
public class FantasticFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if(Headline_REQUESTHTTP_OK==msg.what){
					...
					FragmentUtil.setDataToListView(FantasticFragment.this.getActivity());//给listview设置数据
					FragmentUtil.setImageToViewPager(FantasticFragment.this.getActivity());//给画廊设置数据
		...
		// 2.调用方法从网络上获取数据
		FragmentUtil.getNewsFromHttp(FantasticFragment.this.getActivity(),handler, httpUrl, httpArg,Headline_REQUESTHTTP_OK);
		return view;
	}
